...
#include <string.h>
    ...
    static uint32_t __crc32b(uint32_t hash, uint8_t v) {
        return crc32_table[(uint8_t)hash^v] ^ (hash>>8);
    }
#endif
...
uint32_t crc32(const void* vptr, size_t len) {
    ...
    for (uint64_t v; len >= sizeof v; len -= sizeof v, ptr += sizeof v) {
        ...
        hash = __crc32d(hash,v);
    }
#endif
    for (uint8_t  v; len >= sizeof v; len -= sizeof v, ptr += sizeof v) {
        memcpy(&v, ptr, sizeof v);
